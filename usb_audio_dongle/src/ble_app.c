						...
						const uint8_t *data, uint16_t len)
{
	...
	if (len > sizeof(rx_payload)) {
		LOG_ERR("Payload length %d exceeds maximum %d", len, sizeof(rx_payload));
		return -EMSGSIZE;
	}

	memcpy((struct audio_payload *)&rx_payload, data, len);
	LOG_INF("dongle[%d] rec[%d]: 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", rx_payload.dev_id, len, rx_payload.data[0], 
			rx_payload.data[1], rx_payload.data[2], rx_payload.data[3]);
